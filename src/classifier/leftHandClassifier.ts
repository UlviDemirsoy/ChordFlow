...
export const LEFT_HAND_CLASS_LABELS = [0, 1, 2, 3, 4, 5, 6, 7] as const
export type LeftHandDegree = (typeof LEFT_HAND_CLASS_LABELS)[number]

export interface LeftHandPrediction {
  label: LeftHandDegree
  confidence: number
  margin: number
...
const MODEL_URL = '/models/left_hand_model.onnx?v=unknown-plus-seven-degrees'
...
export class LeftHandClassifier {
  ...
  async predict(
    ...
    return {
      label: LEFT_HAND_CLASS_LABELS[first.index],
      confidence: first.probability,
      margin,
      probabilities,
      accepted:
        first.probability >= CONFIDENCE_THRESHOLD &&
